...
const InventoryDashboard = () => {
  ...
  // Update Inventory Stats dynamically
  const updateInventoryStats = (data) => {
    setTotalProducts(data.length);
    setTotalRevenue(data.reduce((acc, product) => acc + (product.quantity * product.buyingPrice), 0));
    setLowStockCount(data.filter(product => product.quantity > 0 && product.quantity <= product.thresholdValue).length);
  ...
  // Sorting logic (like Orders page)
  ...
  const getStatusColor = (quantity, thresholdValue) => {
    if (quantity === 0) return 'text-red-600'; // Out of stock
    if (quantity <= thresholdValue) return 'text-yellow-400'; // Low stock
    return 'text-blue-500'; // In stock
  };
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mt-3 overflow-x-scroll">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex space-x-2">
            <button className="bg-blue-600 text-white px-4 py-2 rounded-lg" onClick={() => setShowForm(true)}>Add Product</button>
            <button className="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg" onClick={() => setIsSortVisible(true)}>Filters</button>
            <button className="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg" onClick={handleDownload}>Download All</button>
          </div>
        </div>

        {/* Sorting Popup */}
        <SortPopup isVisible={isSortVisible} onClose={() => setIsSortVisible(false)} onSort={handleSort} />
          ...
          <tbody>
            {currentProducts.map((product) => (
              <tr key={product._id}>
                ...
                <td className={`py-3 px-4 text-center border-b border-gray-300 ${getStatusColor(product.quantity, product.thresholdValue)}`}>
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <button className="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages}>Next</button>
        </div>
      </div>

      {/* Add Product Form Modal */}
      {showForm && <AddProductForm onSubmit={() => setShowForm(false)} onDiscard={() => setShowForm(false)} />}
    </div>
  );
};
